...
-export([
    create_key/2, create_key/3,
    create_key_from_data/2,
    create_key_from_table/3,
    st_key/0,
    version_key/0,
...
create_key(Key, TName, Prefix) when is_integer(Prefix) andalso is_atom(TName) ->
    KeyAtom = utils:to_atom(Key),
...
is_visible([], _TName, _Tables, _TxId) ->
    false;
is_visible(Data, TName, Tables, TxId) ->
...
set_version(Element, TxId) ->
    ...
    Version =
        case CurrData of
            [] -> 1;
            _Else -> proplists:get_value(VersionKey, CurrData) + 1
        end,
...
build_fks(Element, Parents) ->
    ...
    lists:foldl(
        fun(?T_FK(FkName, _, FkTable, FkColName, _), AccElement) ->
...
parents(Data, Table, Tables, TxId) ->
    Fks = table:shadow_columns(Table),
    lists:foldl(
        fun(?T_FK(Name, Type, TTName, _, _), Dict) ->
            case Name of
                [ShCol] ->
...
apply_offset(_Key, _AQL, _Constraint, Value) ->
    Value.
...
implicit_state(Table, Data, Tables, [?T_FK(FKName, _, FKTName, _, _) | Fks], TxId) when
    length(FKName) == 1
->
    Policy = table:policy(Table),
