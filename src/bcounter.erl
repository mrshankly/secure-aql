...
check_bcounter_value(?PARSER_GEQ, _Key, OffValue, _) when OffValue >= 0 ->
    OffValue;
check_bcounter_value(?PARSER_LEQ, _Key, OffValue, _) when OffValue >= 0 ->
    OffValue;
check_bcounter_value(?PARSER_GREATER, _Key, OffValue, _) when OffValue >= 0 ->
    OffValue;
check_bcounter_value(?PARSER_LESSER, _Key, OffValue, _) when OffValue >= 0 ->
    OffValue;
...
from_bcounter(?PARSER_GREATER, Value, Offset) ->
    Value + Offset + 1;
from_bcounter(?PARSER_GEQ, Value, Offset) ->
    Value + Offset;
from_bcounter(?PARSER_LESSER, Value, Offset) ->
    -1 * (Value + Offset * -1 + 1);
from_bcounter(?PARSER_LEQ, Value, Offset) ->
    -1 * (Value + Offset * -1).
