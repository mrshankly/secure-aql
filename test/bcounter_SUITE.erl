...
-export([
    greater_insert_basic/1,
    greater_insert_fail/1,
    greater_update_basic/1,
    greater_update_fail/1,
    smaller_insert_basic/1,
    smaller_insert_fail/1,
    smaller_update_basic/1,
    smaller_update_fail/1
]).
...
init_per_suite(Config) ->
    ...
    Query = [
        ...
        TNameGreater,
        " (ID INT PRIMARY KEY, ",
        "bcA COUNTER_INT CHECK (bcA > ",
        BoundGreaterA,
        "), ",
        "bcB COUNTER_INT CHECK (bcB > ",
        BoundGreaterB,
        ")",
        ");",
        "CREATE UPDATE-WINS TABLE ",
    ...
    lists:append(Config, [
        ...
        {update_greater,
            lists:concat([
                "UPDATE ",
                TNameGreater,
                " SET bcA = bcA ~s AND bcB = bcB ~s WHERE ID = ~p"
            ])},
        {update_smaller,
            lists:concat([
                "UPDATE ",
...
all() ->
    [
        greater_insert_basic,
        greater_insert_fail,
        greater_update_basic,
        greater_update_fail,
        smaller_insert_basic,
        smaller_insert_fail,
        smaller_update_basic,
        smaller_update_fail
    ].
...
greater_insert_fail(Config) ->
    BoundA = ?value(bound_greater_a, Config),
    BoundB = ?value(bound_greater_b, Config),
    {_, [{error, Msg1}], _} = tutils:aql(
        ?format(insert_greater, [10, BoundA, BoundB + 1], Config)
    ),
    {_, [{error, Msg2}], _} = tutils:aql(
        ?format(insert_greater, [11, BoundA + 1, BoundB], Config)
    ),
    {_, [{error, Msg3}], _} = tutils:aql(
        ?format(insert_greater, [12, BoundA, BoundB - 1], Config)
    ),
...
greater_update_fail(Config) ->
    ...
    [V1, V2] = tutils:read_keys(TName, Key, ["bcA", "bcB"]),
    % assert value does not change on fail
    ?assertEqual(BoundA + 2, V1),
...
smaller_insert_fail(Config) ->
    ...
    BoundB = ?value(bound_smaller_b, Config),
    {_, [{error, Msg1}], _} = tutils:aql(
        ?format(insert_smaller, [10, BoundA, BoundB - 1], Config)
    ),
    {_, [{error, Msg2}], _} = tutils:aql(
        ?format(insert_smaller, [11, BoundA - 1, BoundB], Config)
    ),
